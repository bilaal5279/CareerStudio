  ...
  const Upload = () => {
    ...
    const fileUpload = async () => {
      ...
      try {
        ...
        const response = await axios.post("/api/convert/resume", formData, {
          headers: {
        ...
        const fileref = ref(storage, `/Resumes/${Upload.name + v4()} + '.txt`);
    ...
    const handleNextClick = async () => {
      if (file) {
        ...
        alert("Please upload a file before clicking Next");
      }
    };
    ...
    return (
      <div className="max-w-screen-xl mt-hero w-full h-screen mx-auto text-center flex flex-col mt-64 text-primary">
        <h1 className="md:text-5xl sm:text-4xl text-3xl font-bold py-10 justi">
          Upload Your Resume
        </h1>
        <div>
          <button
        ...
        {file ? (
          ...
          <p className="text-[#1A2F4F] font-light ">
